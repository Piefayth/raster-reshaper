...
use super::{macros::declare_node, shared::{Vertex, U32_SIZE}, EdgeDataType, ExampleNodeInputs, ExampleNodeOutputs, Field};
    ...
    methods: {
        new(
            ...
            Self {
                ...
                texture_extents: Field::Extent3d(texture_extents),
                texture_format: Field::TextureFormat(texture_format),
                output_image: Field::Image(None),
                triangle_color: Field::Vec4(Vec4::ONE),
                entity,
            }
        }
        process(&mut self, render_device: &CustomGpuDevice, render_queue: &CustomGpuQueue) {
            ...
            if let Field::Vec4(new_color) = self.triangle_color {
            ...
            let input_extents = match self.texture_extents {
                Field::Extent3d(xtnt) => xtnt,
                _ => panic!("Silly developer forgot to uphold this invariant. [input_extents"),
            };
    
            let input_texture_format = match self.texture_format
            {
                Field::TextureFormat(tfmt) => tfmt,
            ...
            self.output_image = Field::Image(Some(image));
        }
    }
);
